const dbLayer = require("../model/hackathon")


let service = {}

service.insertData = () => {
    return dbLayer.insertData().then((data) => {
        return data;
    })
}

service.createOrg = (obj) => {
    return dbLayer.createOrg(obj).then((data) => {
        return data;
    });
}

service.insertSurvey = (obj) => {
    return dbLayer.insertSurvey(obj).then((data) => {
        return data;
    });
}

service.getSurveysByOrg = (orgId) => {
    return dbLayer.getSurveysByOrg(orgId).then((surveyObject) => {
        return surveyObject;
    });
}

service.getSurveyDetails = (surveyId) => {
    return dbLayer.getSurveyDetails(surveyId).then((surveyDetails) => {
